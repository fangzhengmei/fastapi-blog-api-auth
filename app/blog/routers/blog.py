...
@router.post('/{id}/publish', status_code=status.HTTP_200_OK, response_model=schemas.ShowBlog)
def publish(id: int, db: Session = Depends(get_db), current_user: models.User = Depends(oauth2.get_current_user)):
    return blog.publish(id, db, current_user)


@router.post('/{id}/unpublish', status_code=status.HTTP_200_OK, response_model=schemas.ShowBlog)
def unpublish(id: int, db: Session = Depends(get_db), current_user: models.User = Depends(oauth2.get_current_user)):
    return blog.unpublish(id, db, current_user)
